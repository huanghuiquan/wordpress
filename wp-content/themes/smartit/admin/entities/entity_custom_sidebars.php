...
class Thememakers_Entity_Custom_Sidebars {
    ...
    public function get_pages_select($selected = '', $name = '', $id = '') {
        $args = array(
            'show_option_none' => __('No pages', THEMEMAKERS_THEME_FOLDER_NAME),
            'selected' => $selected,
            'echo' => 0,
            'name' => $name,
            'id' => $id
	 ...
	 public function get_products_select($selected = '', $name = '', $id = '') {
        $args = array(
            'post_type' => 'product'
		);
	...
	if ( ! empty($pages) ) {
		...
		$output = "<select name='" .  $name  . "' id='" .  $id  . "'>\n";
				$output .= "\t<option value=\"-1\">No Products</option>";
		
		foreach($pages as $page){
				if (!empty($selected)&&($selected==$page->ID)) 
					$output .= "\t<option selected='selected' value=\"" . $page->ID . "\">$page->post_title</option>\n";
				else		
    ...
    public function add_sidebar() {
		...
		$data['products_select'] = $obj->get_products_select('', "sidebars[" . $sidebar_id . "][product][0]", "sidebars_page_" . $sidebar_id);
   ...
   public function add_sidebar_product() {
        $obj = new Thememakers_Entity_Custom_Sidebars();
        $sidebar_id = $_REQUEST['sidebar_id'];
        $product_id = $_REQUEST['product_id'];
        $data = array();
        $data['select'] = $obj->get_products_select('', "sidebars[" . $sidebar_id . "][product][" . $product_id . "]", "sidebars_product_" . $sidebar_id);
    ...
    public function save_sidebars($newdata, $is_new = false) {
        ...
        update_option(THEMEMAKERS_THEME_PREFIX . "thememakers_sidebars", $sidebars);
    }
                ...
                if ($type == "cat") {
                    ...
                    if (in_array($cat_id, $area['cat'])) {
						...
						if ($post->post_type=='product') {
							 ...
							 if (in_array($post_id, $area['product'])) {
        ...
        if ($show_default_sidebar) {
            if (function_exists('dynamic_sidebar') AND dynamic_sidebar('Thememakers Default Sidebar')):else:
                ...
                <div class="widget widget_categories">
                    <h3 class="widget-title"><?php _e('Categories', THEMEMAKERS_THEME_FOLDER_NAME) ?></h3>
                    <ul class="categories">
                        <?php wp_list_categories('sort_column=name&optioncount=1&hierarchical=0&title_li=0'); ?>
                    </ul>                    
                </div>
                <div class="widget widget_calendar">
                    <h3 class="widget-title"><?php _e('Calendar', THEMEMAKERS_THEME_FOLDER_NAME) ?></h3>
                    <?php get_calendar(); ?>

                </div>

                <div class="widget widget_meta">
                    <h3 class="widget-title"><?php _e('Meta', THEMEMAKERS_THEME_FOLDER_NAME) ?></h3>
                    <ul>
                        <?php wp_register(); ?>
                        <li>
                            <?php wp_loginout(); ?>
